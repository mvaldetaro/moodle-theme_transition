...
This document describes how to copy and customise the Transition (bootstrapbase) theme so that
...
From your Moodle theme directory right click on Transition and then copy and paste back
into your Moodle theme directory. You should now have a folder called Copy of Transition.
...
underscores. For the purpose of this tutorial we will call the theme 'Transitiontheme'.

On opening 'transitiontheme' your you will find several files and sub-directories which have
files within them.
...
/layout/login.php
    Layout file for login screen layout (content only).
    (Contains some elements that require renaming).
/layout/columns1.php
...
where the old theme name occurs, in this case Transition. So using the above list as
a guide, search through and change all the instances of the theme name
'Transition' to 'Transitiontheme'. This includes the filename of the lang/en/theme_Transition.php.
You need to change this to 'theme_Transitiontheme.php'.
...
and then click on (Transitiontheme) or whatever you renamed your theme to,
from the list of theme names that appear at this point in the side block.

Customisation using custom theme settings
-----------------------------------------

The settings page for the Transition theme can be located by navigating to:

Administration > Site Administration > Appearance > Themes > Transition

Moodle documentation
--------------------

Further information can be found on Moodle Docs: http://docs.moodle.org/dev/transition_theme
